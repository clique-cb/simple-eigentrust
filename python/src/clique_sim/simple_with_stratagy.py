...
from .stratagy.basic import TrustStrategy


class SimpleWithParameters(Protocol, Serializable):

    def __init__(self, trust_strategy: TrustStrategy):
    ...
    def _capacity(self, user_id: str, recipient_id: str) -> int:
        return (
            self._graph[user_id][recipient_id]["capacity"]
    ...
    def _flow(self, user_id: str, recipient_id: str) -> int:
        return (
            self._graph[user_id][recipient_id]["flow"]
            if self._graph.has_edge(user_id, recipient_id)
            else 0
        )
    ...
    def available_balance(self, user_id: str) -> int:
        ...
        return (
            ...
            - sum(self.locked(user_id).values())
            + sum(self.debt(user_id).values())
        )

    def edge_credit_limit(self, creditor_id: str, debtor_id: str) -> int:
        self.require_registered(creditor_id)
        return min(
            self.remaining_capacity(creditor_id, debtor_id),
            self.available_balance(creditor_id),
        )

    def credit_limit(self, user_id: str) -> Dict[str, int]:
        self.require_registered(user_id)
        return {
            v: self.edge_credit_limit(v, user_id)
    ...
    def apply_trust_strategy(self, user_id: str):
        """
        Generate and apply trust deltas using the selected trust strategy.
        """
        trust_deltas = self.trust_strategy.generate_trust_deltas(user_id, self._graph)
        self.set_trust(user_id, trust_deltas)
    ...
    def set_trust(self, user_id: str, trust_deltas: Dict[str, int]):
        ...
        for recipient_id, amount in trust_deltas.items():
            if self.remaining_capacity(user_id, recipient_id) + amount < 0:
                raise ValueError(
                    f"Cannot decrease the trust from {user_id} to {recipient_id} by {amount}"
                )
    ...
    def borrow_or_repay(self, user_id: str, flow_deltas: Dict[str, int]):
        ...
        for creditor_id, amount in flow_deltas.items():
            ...
            if new_flow < 0:
                raise ValueError(
                    f"{user_id} cannot repay {-amount} to {creditor_id}: not enough debt"
                )
            if new_flow > self._capacity(creditor_id, user_id):
                raise ValueError(
                    f"{user_id} cannot borrow {amount} from {creditor_id}: not enough capacity"
                )
    ...
    def to_dict(self) -> Dict:
        return {"balances": self._balances, "graph": nx.node_link_data(self._graph)}
